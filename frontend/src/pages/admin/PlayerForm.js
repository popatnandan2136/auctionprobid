...
export default function PlayerForm({ auctionId, onSuccess, onCancel, initialData, showModal }) {
    ...
    React.useEffect(() => {
        if (auctionId) {
            API.get(`/auction/${auctionId}`).then(res => {
                ...
                setAuctionConfig({
                    categories: res.data.customCategories || ["Batsman", "Bowler", "All-Rounder", "Wicket Keeper"],
                    roles: res.data.customRoles || ["Opener", "Middle Order", "Finisher", "Spinner", "Fast Bowler"]
                });
            }).catch(console.error);
        }
    }, [auctionId]);
    ...
    React.useEffect(() => {
        if (initialData?._id) {
            API.get(`/player/${initialData._id}`).then(res => {
